...
public class Sellreport extends JPanel{
	...
	Object rowuser[][];
	String[] columnuser= {"Customer ID","Customer Name","Customer Mobile",
			"Customer Address"};
	...
	private void btnAction() {
		table.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				int col2=0;
				String cusid = model.getValueAt(row, col2).toString();
				tableDetailsDataLoad(id);
				tableCustomerDataLoad(cusid);
	...
	public void tableOrderDataLoad() {
		...
		try {
			...
			String query ="select customerid,sellid,selldate,total "
					+ "from tbsellinfo order by selldate";
	...
	public void tableCustomerDataLoad(String cusid) {
		...
		try {
			...
			String query ="select CustomerID,customername,customermobile,customeraddress "
					+ "from tbcustomerinfo where CustomerID='"+cusid+"'";
			ResultSet rs =DbConnection.sta.executeQuery(query);
			while(rs.next()) {
				modeluser.addRow(new Object[] {rs.getString("CustomerID"),
	...
	public void tableDetailsDataLoad(String id) {
		...
		try {
			...
			String query ="select MedicineID,MedicineName,MedicineType,MfgDate,ExpDate,"
					+ "Qty,Unit,Price,total from tbselldetailsinfo where sellid='"+id+"'";
			ResultSet rs =DbConnection.sta.executeQuery(query);
			while(rs.next()) {
